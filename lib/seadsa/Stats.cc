...
void SeaDsaStats::count_max(const std::string &name, unsigned value) {
  (void)name;
  (void)value;
}
...
ScopedSeaDsaStats::ScopedSeaDsaStats(std::string &&name, const char *suffix,
                                     bool use_count)
    : m_name("") {
  (void)name;
  (void)suffix;
...
unsigned &SeaDsaStats::getCounter(const std::string &name) {
  return getCounters()[name];
}

llvm::raw_ostream &SeaDsaStats::getOutput() { return llvm::errs(); }
...
void SeaDsaStats::count_max(const std::string &name, unsigned value) {
  if (SeaDsaStatsFlag) {
    getCounters()[name] = std::max(getCounters()[name], value);
  }
}
