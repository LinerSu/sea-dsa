...
public:
  ...
  DsaAllocator(bool use_pool = false)
      : m_pool(256 /* block size */, 65536 /* number of blocks to grow by */),
        m_use_pool(use_pool){};
...
Node::Node(Graph &g, const Node &n, bool cpLinks, bool cpAllocSites)
    : m_graph(&g), m_unique_scalar(n.m_unique_scalar), m_size(n.m_size),
      m_arrayMinSize(n.m_arrayMinSize), m_arrayMaxSize(n.m_arrayMaxSize) {
...
unsigned Node::Offset::getNumericOffset() const {
  ...
  for (auto &ck : n->getCollapsedCells()) {
    if (ck.includes(offset)) { return ck.getOffset(); }
  }
  return offset;
}
...
void Node::unifyAt(Node &n, unsigned o) {
  ...
  if (!isOffsetCollapsed() && !n.isOffsetCollapsed() && n.isArray() &&
    ...
    /*
    Sequence Node n (array, size=stride=4,arraySize=m):
                        +-----------------------+
...
unsigned Cell::getOffset() const {
  ...
  if (isNull() || getNode()->isOffsetCollapsed())
    ...
    return (it != cells.end()) ? it->getRawStartOffset() : offset;
  }
}
...
void Graph::import(const Graph &g, bool withFormals) {
  ...
  for (auto &kv : g.m_values) {
    ...
    Cell *nc = nullptr;
    {
      SEADSA_SCOPED_STATS("interval_graph.import.values.mkcell", 1);
      nc = &mkCell(*kv.first, Cell());
    }

    // -- unify the old and new cells
    {
      SEADSA_SCOPED_STATS("interval_graph.import.values.unify", 1);
      nc->unify(c);
    }
  }
  ...
  if (withFormals) {
    for (auto &kv : g.m_formals) {
      ...
      {
        SEADSA_SCOPED_STATS("interval_graph.import.formals.unify", 1);
        nc->unify(c);
      }
    }
    for (auto &kv : g.m_returns) {
      ...
      {
        SEADSA_SCOPED_STATS("interval_graph.import.returns.unify", 1);
        nc->unify(c);
      }
    }
  }
  ...
  {
    SEADSA_SCOPED_STATS("interval_graph.import.compress", 1);
    compress();
  }
}
