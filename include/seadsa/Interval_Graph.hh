...
private:
  ...
  collapsed_cells_type
      m_collapsedCells; ///< Set of collapsed interval cells in this node

  unsigned m_size; ///< Size of the memory object in bytes; array stride for
                   ///< array nodes
  boost::optional<unsigned>
      m_arrayMinSize; ///< Known lower bound for array extent in bytes
  boost::optional<unsigned>
...
public:
  ...
  /**
   * @brief Set this node as an array with the given stride
   *
  ...
  Node &setArraySize(unsigned sz) {
    return setArraySize(sz, boost::none, boost::none);
  }

  /**
   * @brief Set this node as an array with stride and optional extent bounds
   *
   * The existing m_size value remains the array stride used for modulo offset
   * adjustment. minSize and maxSize track statically-known bounds on the array
   * extent in bytes when available.
   *
   * @param sz Array stride in bytes
   * @param minSize Known lower bound for array extent in bytes, if any
   * @param maxSize Known upper bound for array extent in bytes, if any
   * @return Reference to this node
   */
  Node &setArraySize(unsigned sz, boost::optional<unsigned> minSize,
                     boost::optional<unsigned> maxSize) {
    assert(!isArray());
    assert(!isForwarding());
    assert(m_size <= sz);
    assert(!minSize || !maxSize || minSize.get() <= maxSize.get());

    setArray(true);
    m_size = sz;
    m_arrayMinSize = minSize;
    m_arrayMaxSize = maxSize;
    return *this;
  }

  /// @brief Set statically-known array extent bounds without changing stride.
  Node &setArrayBounds(boost::optional<unsigned> minSize,
                       boost::optional<unsigned> maxSize) {
    assert(isArray());
    assert(!minSize || !maxSize || minSize.get() <= maxSize.get());
    m_arrayMinSize = minSize;
  ...
  boost::optional<unsigned> getArrayMinSize() const { return m_arrayMinSize; }
  boost::optional<unsigned> getArrayMaxSize() const { return m_arrayMaxSize; }
  bool hasArrayMinSize() const { return static_cast<bool>(m_arrayMinSize); }
  bool hasArrayMaxSize() const { return static_cast<bool>(m_arrayMaxSize); }
