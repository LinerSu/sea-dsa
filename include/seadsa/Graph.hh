 ...
 * - Graph: Container for DSA nodes and mappings from LLVM values to memory cells
...
/**
 * @class Graph
 * @brief Represents a DSA graph containing memory objects (nodes) and their relationships
...
protected:
  ...
  using CallSites = std::vector<std::unique_ptr<DsaCallSite>>;  /// Indirect call sites owned by this graph
  ///
...
class Field {
  ...
  FieldType m_type = FIELD_TYPE_NOT_IMPLEMENTED;  ///< Type information for this field
...
class Cell {
protected:
  mutable Node *m_node = nullptr;    ///< The memory object (node) this cell refers to
 ...
 * The overlapped means pointers refer fields that in the chunk space are may alias.
 * A chunk is essentially a pair (Node*, start, end) that references a specific
 * chunk of fields within a memory object. 
 * Chunk may grow when overlapped range expands.
 * Each chunk maintains:
 * - Node* m_node: The memory object (node) this chunk belongs to
 * - start offset: the start offset that indicate the following fields are collapsed
...
protected:
  mutable Node *m_node = nullptr;    ///< The memory object (node) this cell refers to
  mutable unsigned m_start = 0;     ///< Byte offset within the node, start of the chunk
  mutable boost::optional<unsigned> m_end = boost::none;       ///< Byte offset within the node, end of the chunk, default is infty
...
public:
  ...
  bool isDisjoint(const Chunk &o) const {
    if (m_node != o.m_node)
      return true;
    if (!m_end && !o.m_end) return false;  // Both infinite ranges are not disjoint
...
private:
  ...
  mutable const llvm::Value *m_unique_scalar; ///< Unique scalar if node has exactly one
...
private:
  accessed_types_type m_accessedTypes;  ///< Map from offset to types accessed at that offset
  ...
  /// @todo This is ugly. IDs should probably be unique per-graph, not unique overall.
  ...
  /**
   * @brief Transfer links/types to another node and make this node forward to it
   * 
   * Transfers all information from this node to the target node at the specified
   * offset, then makes this node a forwarding pointer. May cause collapse if
   * unification cannot preserve field sensitivity.
   * 
   * @note Most clients should use unifyAt() which has less stringent preconditions
...
public:
   ...
   * @return Bitmask: 0x1 if this changed, 0x2 if n changed, 0x3 if both changed, 0x0 if no change
   */
   ...
   * @param n Node to merge with
   * @return Change bitmask (0x1=this changed, 0x2=n changed, 0x3=both, 0x0=none)
   */
